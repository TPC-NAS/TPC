...
class MasterNet(PlainNet.PlainNet):
    ...
    def get_FLOPs(self, input_resolution):
        ...
        if self.dataset in ['imagenet', 'myimagenet100']:
            the_flops += self.fc_linear_1.get_FLOPs(the_res)
            the_flops += self.fc_linear_2.get_FLOPs(the_res)
        else:
    ...
    def get_model_size(self):
        ...
        if self.dataset in ['imagenet', 'myimagenet100']:
            the_size += self.fc_linear_1.get_model_size()
            the_size += self.fc_linear_2.get_model_size()
        else:

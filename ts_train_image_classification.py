...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, opt, num_train_samples, no_acc_eval=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        ts_feature_loss, ts_logit_loss = model.module.compute_ts_distill_loss()
...
def validate(val_loader, model, criterion, opt, epoch='N/A'):
    ...
    sync_tensor = torch.tensor([top1.sum, top1.count, top5.sum, top5.count], dtype=torch.float32).cuda(opt.gpu)
    # sync_tensor = dist.all_reduce(sync_tensor, average=False, name='sync_tensor_topk_acc')
...
def main(gpu, ngpus_per_node, opt, argv):
    ...
    opt.gpu        = gpu
    opt.local_rank = opt.gpu
    opt.batch_size = opt.batch_size_per_gpu

    global_rank = opt.gpu + opt.machine_rank * ngpus_per_node
    ...
    comm.synchronize()
    opt.rank = comm.get_rank() # global rank
    ...
    if opt.rank == 0:
        ...
        setup_logging(log_filename, mode='w')
    else:
    ...
    if teacher_opt.load_parameters_from:
        teacher_model = load_model(teacher_model, teacher_opt.load_parameters_from, teacher_opt.strict_load,map_location='cpu')
    ...
    model.cuda(opt.gpu)
    model = comm.get_parallel_model(model, opt.gpu) #local rank
...
if __name__ == "__main__":
    ...
    opt.dist_url ="tcp://127.0.0.1:10000"
    opt.num_machines = 1
    opt.machine_rank = 0
    opt.num_nodes    = 1
    ...
    print("GPU to use = ", ngpus_per_node)
    opt.world_size = ngpus_per_node * opt.num_nodes

    mp.spawn(main, nprocs=ngpus_per_node, args=(ngpus_per_node, opt, sys.argv))
    # main(opt, sys.argv)
